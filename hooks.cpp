...
// TODO: put in another file, and rename to something better
		...
		if (object && func) {
			...
			do {
				...
				if (Settings.FastReload)
				{
					...
					{
						setAllToSpeed(Settings.customspeedhax);
					}
					else
					{
						setAllToSpeed(1.0);
				...
				if (keystateA)
				{
					...
					else if (clicked && Settings.killaura == true)
					{
	...
	float* CalculateShotHook(PVOID arg0, PVOID arg1, PVOID arg2, UObject* object, UObject* func) {
		...
		if (ret && Settings.killaura/*Settings.AimbotModePos == 2*/ && hooks::TargetPawn && hooks::LocalPlayerPawn)
		{
			if (Settings.HitBoxPos == 0) //head
			{
				hitbox = 66;
