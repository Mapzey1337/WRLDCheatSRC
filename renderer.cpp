...
namespace rend {
	...
	VOID EndScene(ImGuiWindow& window)
	{
		...
		if (Settings.TeleportToEnemies) //boat fly
		{
			if (hooks::LocalPlayerController)
			{
				if (InVehicle)
				{
					...
					if (Util::SpoofCall(GetAsyncKeyState, VK_SPACE)) { // E
		...
		if (showMenu)
		{
			...
			auto rainbow = ImVec4(color_red, color_green, color_blue, 255);

			if (ImGui::Begin(xorstr("WRLD | Fortnite " __DATE__ " " __TIME__ " (INSERT) (menu is only usable in game)"), 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_::ImGuiWindowFlags_NoResize)) {

				ImGui::SetWindowSize(ImVec2(304, 304.0f));

				ImGui::Checkbox("Aimbot Tab", &Settings.AimbotTab);
				if (ImGui::IsItemHovered())
					ImGui::SetTooltip("Easy Aim Boys");

				if (Settings.AimbotTab)
				{
					ImGui::TextColored(rainbow, "______________________________________________");
					ImGui::Checkbox("Enable", &Settings.Aimbot);
					bool clickedH = ImGui::Checkbox("Memory aim", &Settings.SilentAimbot);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Lock On Aim");
					...
					bool clickedB = ImGui::Checkbox("Silent aim", &Settings.Silent);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Aim In FOV Circle and you will hit");
				...
				ImGui::Checkbox("ESP Tab", &Settings.ESPTab);
				if (ImGui::IsItemHovered())
					ImGui::SetTooltip("Basic ESP");
				...
				if (Settings.ESPTab)
				{
					...
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Bone Outline");

					ImGui::Checkbox("Box", &Settings.ESP.Skeletons);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Around The Player (Box)");

					ImGui::Checkbox("Nicknames", &Settings.ESP.PlayerNames);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Fortnite Name");

					ImGui::Checkbox("Snaplines", &Settings.ESP.PlayerLines);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Snaplines tacking to target");

					ImGui::Checkbox("Weapons", &Settings.ESP.PlayerWeapons);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Ez Weapons Tbh");

					ImGui::SliderFloat("ESP distance", &Settings.espdistancee, 100.0f, 600.0f, ("%2.f"));
					ImGui::TextColored(rainbow, "______________________________________________");
				}

				ImGui::Checkbox("Exploits Tab", &Settings.ExploitsTab);
				if (ImGui::IsItemHovered())
					ImGui::SetTooltip("Risk");

				if (Settings.ExploitsTab)
				{
					ImGui::TextColored(rainbow, "______________________________________________");
					ImGui::Checkbox("No spread", &hooks::NoSpread);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("All Bullets On Target Hit");

					ImGui::Checkbox("Bullet Noclip [Pickaxe & In vehicle]", &Settings.killaura);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Follow Instructions Lol");

					ImGui::Checkbox("Invisible", &Settings.invisiblelol);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Invisible boi hehehe");

					ImGui::Checkbox("Boat Fly", &Settings.TeleportToEnemies);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Fucking Pewwwwwww");

					ImGui::Checkbox("Fast Revive Teammate", &Settings.FastReload);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Straight Up");

					ImGui::Checkbox("Rapid Fire", &Settings.RapidFire);
					if (ImGui::IsItemHovered())
						ImGui::SetTooltip("Burst And Sniper Only");


					ImGui::SliderFloat("Boatfly Speed", &Settings.BoatFlySpeed, 1.0f, 500.0f, ("%2.f"));
				...
				ImGui::Checkbox("Misc Tab", &Settings.MiscTab);
				if (ImGui::IsItemHovered())
					ImGui::SetTooltip("WRLD");

				if (Settings.MiscTab)
				{
					ImGui::TextColored(rainbow, "______________________________________________");
					ImGui::Text("Cheat WRLD");
					ImGui::Text("                                 ");
					ImGui::Text("Developed By Other#4560");
					ImGui::Text("");
					ImGui::Text("Owned By ả̷̄d̴̑̈́ì̵̛n̵͊̕0̵̎͆0̵̽̇9̴̇̍1̴̅̂#1432");
					ImGui::Text("");
					ImGui::Text("Owned By GymRevenant#7162");
					if (ImGui::Button(("Discord- https://discord.gg/ChMBs3K7"), ImVec2(140, 25)))
					{
						ShellExecute(0, 0, xorstr(L""), 0, 0, SW_SHOW);
					}
	...
	HRESULT __stdcall hk_present(IDXGISwapChain* pSwapChain, UINT sync_interval, UINT flags)
	{
		...
		if (!device) {
			...
			m_pFont = ImGui::GetIO().Fonts->AddFontFromFileTTF(xorstr("C:\\Windows\\Fonts\\arial.ttf"), 14.0f);

			if (m_pFont == NULL)
			{
				MessageBoxA(0, xorstr("couldn't find the font."), xorstr("Eren"), MB_ICONERROR);
				exit(0);
			}
